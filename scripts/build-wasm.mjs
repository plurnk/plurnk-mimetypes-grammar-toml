#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-toml.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-toml-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter-grammars/tree-sitter-toml.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as toml.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "toml.wasm"));
const bytes = (await readFile(path.join(repoRoot, "toml.wasm"))).length;
console.log(`toml.wasm: ${bytes} bytes (built from ${pin})`);
