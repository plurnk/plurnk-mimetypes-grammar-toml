#!/usr/bin/env node
// CI byte-identical reproducibility check for toml.wasm.
...
const committed = await readFile(path.join(repoRoot, "toml.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed toml.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-toml-verify-"));
...
console.log(`rebuilt toml.wasm sha256: ${rebuiltHash}`);
